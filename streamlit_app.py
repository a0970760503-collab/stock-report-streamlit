...
def render_broker_report_analysis_app(mode="admin"):
    is_admin = mode == "admin"
    st.title("📈 券商研究報告分析工具")
    if is_admin:
        st.caption("上傳券商 PDF 研究報告，自動擷取股票、券商、評等、目標價與報告日期，彙整成可比較的分析表。")
    else:
        st.caption("券商研究報告彙整分析。")

    uploaded_rows = st.session_state.get("auto_rows", []) if is_admin else []
    ...
    with st.sidebar:
        ...
        use_sample = st.checkbox("使用範例資料", value=not bool(uploaded_rows or database_rows)) if is_admin else False
    ...
    pending_rows = pending_report_rows() if is_admin else []
    ...
    if is_admin:
        tab_names.append("📝 待確認")
    tabs = st.tabs(tab_names)
    overview_tab, stock_tab, detail_tab, portfolio_tab = tabs[:4]
    pending_tab = tabs[4] if is_admin else None
